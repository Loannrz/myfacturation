...
type QuoteLine = { description: string; quantity: number; unitPrice: number; vatRate: number; discount: number }
...
export default function ModifierDevisPage() {
  ...
  const [lines, setLines] = useState<QuoteLine[]>([{ description: '', quantity: 1, unitPrice: 0, vatRate: 20, discount: 0 }])
  ...
  useEffect(() => {
    ...
    Promise.all([
    ...
    ]).then(([quote, clientsList, companiesList, productsList, settings]) => {
      if (!quote) {
        setNotFound(true)
      ...
      setEmitterProfileId(quote.emitterProfileId ?? (profiles[0]?.id ?? ''))
      setClientId(quote.clientId ?? '')
      setCompanyId(quote.companyId ?? '')
      setRecipientType(quote.clientId ? 'client' : quote.companyId ? 'company' : 'client')
      setIssueDate(quote.issueDate?.slice(0, 10) ?? new Date().toISOString().slice(0, 10))
      const issue = quote.issueDate?.slice(0, 10)
      const due = quote.dueDate?.slice(0, 10)
      if (issue && due) {
  ...
  if (notFound) {
    return (
      <div className="max-w-6xl mx-auto">
        ...
        <div className="p-8 text-center text-[var(--muted)]">Devis introuvable.</div>
      </div>
    )
  }
