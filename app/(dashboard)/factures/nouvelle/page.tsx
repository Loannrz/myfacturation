...
export default function NouvelleFacturePage() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (hasEmptyLine) {
      setFormError('Impossible de créer la facture : supprimez les lignes vides (seules les lignes avec une description sont autorisées).')
      return
    }
    setLoading(true)
    try {
      ...
      if (res.ok) {
        ...
        router.push(`/factures?created=${inv.id}`)
      } else {
        const data = await res.json().catch(() => ({}))
    ...
    return (
      <div className="max-w-2xl mx-auto">
        ...
        <div className="border border-amber-500/50 rounded-xl p-6 bg-amber-500/10 flex gap-4">
          ...
          <div>
            <h1 className="text-xl font-semibold text-amber-800 dark:text-amber-200">Impossible de créer une facture</h1>
  ...
  return (
    <div className="max-w-6xl mx-auto">
      ...
      <h1 className="text-2xl font-semibold text-[var(--foreground)] mb-2">Nouvelle facture</h1>
      ...
      <div className="space-y-8">
        ...
        {limitReached && (
          <div className="p-4 rounded-xl border border-violet-500/50 bg-violet-500/10 flex flex-wrap items-center justify-between gap-3">
            <p className="text-sm text-violet-800 dark:text-violet-200">Passez à Pro pour créer des factures illimitées.</p>
