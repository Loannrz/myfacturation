...
export default function ModifierSocietePage() {
  ...
  useEffect(() => {
    ...
    fetch(`/api/companies/${id}`)
      .then((r) => {
        if (!r.ok) throw new Error('Société introuvable')
        return r.json()
      })
      .then((c: CompanyData) => {
        setType((c.type === 'association' ? 'association' : 'societe') as 'societe' | 'association')
        setName(c.name ?? '')
        ...
        setSiret(c.siret ?? '')
        setVatExempt(!!c.vatExempt)
        setVatNumber(c.vatNumber ?? '')
        setEmail(c.email ?? '')
        setPhone(c.phone ?? '')
        setWebsite(c.website ?? '')
      })
      .catch(() => setError('Société introuvable'))
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (res.ok) {
        router.push('/societes?updated=' + id)
      } else {
  ...
  if (loadingCompany) {
    return (
      <div className="max-w-xl mx-auto">
        ...
        <p className="text-[var(--muted)]">Chargement de la société…</p>
      </div>
    )
  }
  ...
  return (
    <div className="max-w-xl mx-auto">
      ...
      <h1 className="text-2xl font-semibold text-[var(--foreground)] mb-6">Modifier la société</h1>
