...
export function BillingSuccessUI({ syncResult, sessionId }: { syncResult: SyncResult; sessionId: string | null }) {
  ...
  useEffect(() => {
    if (didUpdate.current || !syncResult.ok) return
    didUpdate.current = true
    updateSession?.({ subscriptionPlan: syncResult.plan, billingCycle: syncResult.cycle }).catch(() => {})
  }, [syncResult.ok, syncResult.plan, syncResult.cycle, updateSession])
