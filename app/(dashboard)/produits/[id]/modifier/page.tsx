...
export default function ModifierProduitPage() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (res.ok) router.push('/produits')
      else {
  ...
  return (
    <div className="max-w-xl mx-auto">
      ...
      <h1 className="text-2xl font-semibold text-[var(--foreground)] mb-6">Modifier le produit</h1>
      ...
      <form onSubmit={handleSubmit} className="space-y-4 border border-[var(--border)] rounded-xl p-6 bg-[var(--background)]">
        ...
        <div className="flex flex-nowrap gap-3 items-end">
          <div className="min-w-0 flex-1">
            ...
            <input type="number" min={0} step={0.01} value={unitPrice} onChange={(e) => setUnitPrice(e.target.value)} className={inputClass} required />
          </div>
          <div className="min-w-0 flex-1">
            <label className="block text-sm text-[var(--muted)] mb-1">TVA %</label>
            <input type="number" min={0} max={100} step={0.1} value={vatRate} onChange={(e) => setVatRate(e.target.value)} className={inputClass} />
          </div>
          <div className="min-w-0 flex-1">
            <label className="block text-sm text-[var(--muted)] mb-1">Remise %</label>
            <input type="number" min={0} max={100} step={0.1} value={discount} onChange={(e) => setDiscount(e.target.value)} className={inputClass} />
          </div>
          <div className="min-w-0 flex-1">
