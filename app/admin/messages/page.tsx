      ...
      {loading ? (
        ...
        <div className="rounded-xl border border-[var(--border)] overflow-hidden">
          <table className="w-full text-sm">
            ...
            <tbody>
              {conversations.map((c) => (
                <tr key={c.id} className="border-t border-[var(--border)] hover:bg-[var(--border)]/10">
                  <td className="p-3">
                  ...
                  <td className="p-3 text-center">
                    {c.unreadCount > 0 ? (
                      <span className="inline-flex items-center justify-center min-w-[1.5rem] h-6 px-1.5 rounded-full bg-amber-500 text-amber-950 text-xs font-medium">
                        {c.unreadCount}
                      </span>
                    ) : (
                      '—'
